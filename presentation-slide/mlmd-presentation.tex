...
\begin{frame}{The SickKids dataset}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{block}{Diagnosis field}
        \small
        Free-text \texttt{dx} column with semicolon-separated
        diagnoses (manual clinician input).
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Relevant work}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item \textbf{Yao et al.\ (2025):} LLMs can impute
              diagnosis$\to$test associations; evaluation-based
        ...
        \item \textbf{Cao et al.\ (2026):} LLM-powered knowledge
              ...
              (JAMIA Open).
        \item \textbf{Islam et al.\ (2025):} LLM prompt ensembles
              extract medical entities from EHR free text.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Noise-robust training}
      \begin{itemize}
        \item \textbf{Moturu, Goldenberg \& Taati (2025):}
              LiLAW --- 3 learnable parameters adapt per-sample
              loss weights via bilevel optimisation. SOTA on
              noisy medical imaging benchmarks.
        \item \textbf{Ren et al.\ (2018):} Meta-learning to
              reweight training examples; foundational approach
              for learning from noisy labels.
        \item \textbf{Northcutt et al.\ (2021):} Confident
              Learning --- characterising and filtering
              label errors using predicted probabilities.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Two complementary strategies}
  ...
  \begin{columns}[c]
    \begin{column}{0.44\textwidth}
      \begin{block}{\centering Track 1 --- Label imputation}
        \small
    ...
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{block}{\centering Track 2 --- LiLAW training}
        \small
...
\begin{frame}{KG track: end-to-end pipeline}
  \centering
  \begin{tikzpicture}[
...
\begin{frame}[fragile]{KG track: from rules to enriched graph}
  \begin{columns}[T]
        ...
        \begin{lstlisting}[language={}]
{
  ...
  "source": "AHA/ACC"
    ...
    \begin{column}{0.49\textwidth}
      \vspace{0.2em}
      \textbf{Enrichment pipeline} {\scriptsize(10 steps)}\\[0.4em]
      \scriptsize
      \begin{tabular}{@{}cll@{}}
        \toprule
...
\begin{frame}{KG track: validating LLM-generated rules}
  ...
  \begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners, fill=lightgray,
                ...
                font=\scriptsize},
    annot/.style={font=\tiny, text width=2.2cm, align=center,
                  text=black!70},
    arr/.style={-{Latex[length=2.5mm]}, thick, color=midblue},
  ]
    ...
    % Annotations below each node
    \node[annot, below=0.15cm of pre]
      {Drop missing fields,\\unknown sources,\\exact duplicates};
    \node[annot, below=0.15cm of norm]
      {Map to ICD-10 (UMLS)\\and HP/MONDO (OLS4)\\before LLM review};
    \node[annot, below=0.15cm of ground]
      {Europe PMC counts\\+ web snippets;\\tag by evidence tier};
    \node[annot, below=0.15cm of verify]
      {LLM reviews batches\\of $\leq$15; evidence tier\\sets retention bias};
  \end{tikzpicture}
\end{frame}

\begin{frame}{KG track: Diagnotix web application}
  \centering
  \includegraphics[width=0.85\linewidth]{images/webapp-ss.png}
\end{frame}
...
\begin{frame}{LiLAW track: learning to down-weight noise}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Core idea}
      \begin{itemize}
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{block}{Properties}
        ...
        \begin{itemize}
          ...
          \item No clean validation set required
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Evaluation plan}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Additional validation of KG track}
...
% ── Limitations ──────────────────────────────────────────────────────────────
\begin{frame}{Limitations and risk factors}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
% ── Audit detail ─────────────────────────────────────────────────────────────
\begin{frame}{Audit detail --- evidence grounding and verification}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}{10-step enrichment pipeline (full)}
  \scriptsize
  \begin{tabular}{@{}clll@{}}
    \toprule
...
\begin{frame}{Label imputation pipeline (detail)}
